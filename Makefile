...
build-all-images: build-dir build-all-prod
...
# save images
#
save-images: build-dir save-image-db save-image-web save-image-back ## Save images

save-image-web: ## Save web image
	web_image_name=$$(${DC} -f $(DC_APP_BUILD_FRONT_PROD) config | python -c 'import sys, yaml, json; cfg = json.loads(json.dumps(yaml.load(sys.stdin), sys.stdout, indent=4)); print cfg["services"]["web"]["image"]') ; \
...
save-image-db: ## Save db image
	db_image_name=$$(${DC} -f $(DC_APP_BUILD_BACK_PROD) config | python -c 'import sys, yaml, json; cfg = json.loads(json.dumps(yaml.load(sys.stdin), sys.stdout, indent=4)); print cfg["services"]["db"]["image"]') ; \
...
save-image-back: ## Save back image
	back_image_name=$$(${DC} -f $(DC_APP_BUILD_BACK_PROD) config | python -c 'import sys, yaml, json; cfg = json.loads(json.dumps(yaml.load(sys.stdin), sys.stdout, indent=4)); print cfg["services"]["back"]["image"]') ; \
...
# clean image
#
clean-images: clean-image-web clean-image-back clean-image-db ## Remove all docker images
...
clean-image-back: ## Remove back docker image
	back_image_name=$$(${DC} -f $(DC_APP_BUILD_BACK_PROD) config | python -c 'import sys, yaml, json; cfg = json.loads(json.dumps(yaml.load(sys.stdin), sys.stdout, indent=4)); print cfg["services"]["back"]["image"]') ; \
          docker rmi $$back_image_name || true
...
publish-$(LATEST_VERSION):
	...
	( cd $(BUILD_DIR) ;\
	    for file in \
                ...
                $(FILE_IMAGE_WEB_LATEST_VERSION) \
                $(FILE_IMAGE_BACK_LATEST_VERSION) \
                $(FILE_IMAGE_DB_LATEST_VERSION) \
           ; do \
