...
until [ "$timeout" -le 0 -o "$test_result" -eq "0" ] ; do
	${DC} -f ${DC_APP_RUN_ALL_PROD}  exec ${USE_TTY} $container_name curl --retry-max-time 120 --retry-delay 1  --retry 1 -s -X POST  http://localhost:8000/api/users/login | grep '"message":'
	test_result=$?
